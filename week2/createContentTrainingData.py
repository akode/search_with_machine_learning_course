...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool() as p:
        ...
        df = pd.DataFrame.from_records([item for label_list in all_labels for item in label_list], columns=["cat", "name"])
        categories = df["cat"].value_counts()>min_products
        
        filtered_df = df[df["cat"].isin(categories.index[categories])]
        print(df.shape)
